...
class TrafficClassyCNN(object):
    ...
    def save(self, saver, sess, epochs, path='./model/'):
        '''
        Saves the trained CNN model to the given relative path.

        :param saver: The tensorflow Saver object.
        :param sess: The tensorflow session.
        :param epochs: The number of epochs the model was trained on.
        :param path: The relative path to save the model into.
        :return: A saved machine learning model.
        '''

        if not os.path.isdir(path):
            os.makedirs(path)
        print('\n[+] Saving model in %s' % path)
        saver.save(sess, os.path.join(path, 'cnn-model.ckpt'), global_step=epochs)

    def load(self, saver, sess, path, epochs):
        '''
        Loads a saved machine learning model.

        :param saver: The tensorflow Saver object.
        :param sess: The tensorflow session.
        :param path: The path to load the trained model from.
        :param epochs: The number of epochs the model was trained on.
        :return: The restored machine learning model in its previous state.
        '''
    ...
    def predict(self, sess, return_proba=False):
        ...
        :param sess: The tensorflow session.
        :param return_proba: A flag to return the prediciton probabilities or to return the prediction labels.
        :return: Either the prediciton probabilities or the prediction labels. This is contingent on the
                 return_proba flag.
        '''
...
if __name__ == "__main__":
    ...
    with tf.Session(graph=g) as sess:
        ...
        cnn.save(saver=saver, sess=sess, epochs=20)
        preds = cnn.predict(sess=sess, return_proba=False)
        print("[+] Test Accuracy: %.3f%%" % (100 * np.sum(preds == cnn.y_test) / len(cnn.y_test)))
        # writer.close()
    del g
