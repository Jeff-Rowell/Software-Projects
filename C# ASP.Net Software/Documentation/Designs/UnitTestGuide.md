...
`[TestFixture]` declares a class as a test fixture. This is needed for all test classes as it signals to the NUnit framework that the class contains tests.
...
Assert.That(testData, <constraint>);

//Test objects are equal
Assert.That(1, Is.EqualTo(1));
Assert.That(testString, Is.EqualTo(expectedString);

//Test for an empty string, directory, or collection.
...
// Any assertion can have an optional third parameter string that will be printed if the assertion fails.
Assert.That(displayName, Is.Not.Empty, "The display name was never initialized");
```
...
Moq is a mocking library for use in testing units seperate from their dependencies which can not or should not be under test. Unit tests should be isolated to the object or class under testing, not any dependencies of the object. Mocks are objects created by the unit test that can stand in as a dependency allowing the unit test to isolate the testing object's behavior.
...
For example, if you are testing a Car object which has an Engine and the Engine isn't implemented yet, the Car needs a Mock Engine in order to test the Car. Mock information about the Engine can be returned by this mock or even mock the state of the Engine. 

In CWMasterTeacher Moq will be useful for testing the Repositories and anything that interacts directly with the database.
...
The object being tested (Assume the Engine methods used are not implemented yet.)
...
public class TestCar
{
    ...
    // The following attributes are equivalent to calling CarFuleTest(25, false), then CareFuleTest(5, true).
    [TestCase(25, false)]
	[TestCase(5, true)]
...
CWTesting
 -->Tests
   -->CWMasterTeacher3
     ...
     -->DbObjects
       -->ClassSectionTest.cs
   -->CWMasterTeacherDataModel
```

The Tests directory should follow the directory structure of the CWMasterTeacher projects. Tests for Factories should go under the Factories folder, etc. If the folder is missing please create it. For each class that has tests written for it, the tests should be contained in a file named after the class and appending the name Test to it. Eg. the WebUtilities.cs class has its tests located under WebUtilitiesTest.cs.
...
Unit tests should test behavior, not necessarily methods. For example it makes little sense to getter and setter methods. Many tests will require a combination of method calls so having a test for each method may not make sense.
...
To run the code coverage tool on a windows based machine open the CWMasterTeacher3 directory and run GenerateCoverageReport.bat. This will create the report and open it in your web browser.

The reports are located in the folder called CoverageReport, you can open the report later by opening the index.htm file inside the folder.

The GenerateCoverageReport.bat creates the reports by doing the following:

1. Call the Visual Studio command to set up the environment variables needed for msbuild.
2. Run the msbuild Coverage target.
  * Since the Coverage target depends on the BuildSolution target to generate the reports the project must be able to build successfully using msbuild.exe. 
3. Open the index.htm file in a web browser.


### Running code coverage by hand

If you are having trouble with the GenerateCoverageReport.bat you can run the necessary commands by hand. There is another file called MSBuildCommandPrompt.bat which will open a command prompt and call the Visual Studio command to set up the environment variables needed for msbuild.

From the new command window you can then run the following command to generate the coverage report:

```
msbuild CWMasterTeacher3.msbuild /t:Coverage
```

The output will show you any errors encountered during the build process. If you need to capture this output you can run the following command which will capture the output to a file named output.txt:
```
msbuild CWMasterTeacher3.msbuild /t:Coverage > output.txt
```
