...
/*
 * Exercise 5-14, 15, and 16.
 *
 * Modify the sort program to handle a -r flag, which indicates sorting in reverse
 * (decreasing) order. Be sure that -r works with -n. Add the option -f to fold upper
 * and lower case together, so that case distinctions are not made during sorting;
 * for example, a and A compare equal. Add the -d (``directory order'') option, which
 * makes comparisons only on letters, numbers and blanks. Make sure it works in conjunction
 * with -f.
 */
